...
use Flat3\RevPi\Hardware\Connect5;
...
class Connect5Test extends TestCase
{
    ...
    public function test_analog_watch_high_low(): void
    {
        ...
        $this->loop(3);
        $this->assertTrue($this->switch);

        $module->image()->writeVariable('AnalogOutputLogicLevel_1', false);
        $this->loop(3);
        $this->assertFalse($this->switch);
    }
    ...
    public function test_analog_voltage(): void
    {
        ...
        $this->loop(1, 100);
        $this->assertEquals(0, $module->image()->readVariable('AnalogOutput_3'));
        $this->assertEqualsWithDelta(0, $module->image()->readVariable('AnalogInput_3'), 100);
        $module->image()->writeVariable('AnalogOutput_3', 2000);
        $this->loop(1, 100);
        $this->assertEqualsWithDelta(2000, $module->image()->readVariable('AnalogInput_3'), 100);
    }
    ...
    public function test_digital_watch_high_low(): void
    {
        ...
        $this->assertFalse($module->image()->readVariable('DigitalOutput_3'));
